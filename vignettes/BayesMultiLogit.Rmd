...
The package contains four wrapper functions for performing Bayesian multinomial logistic regression. These wrappers validate dimensions, response values, iteration counts, flags, and prior parameters before calling the C++ samplers.

Each function expects matrix-like `Y` and `X` inputs. `Y` is an $N$ by $C$ matrix with one row per observation, and `X` is an $N$ by $P$ design matrix. The Metropolis-Hastings, elliptical slice, and Polya-Gamma samplers accept non-negative category counts with a positive row total. The Holmes-Held sampler requires one-hot rows. The first column of `X` should be an intercept column of ones; centering and scaling the remaining columns is generally helpful.
